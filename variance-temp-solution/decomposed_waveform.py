...
class DecomposedWaveformPyWorld(DecomposedWaveform):
    ...
    def _world_extraction(self):
        ...
        x = (
            self._waveform.astype(np.double)
            + np.random.randn(*self._waveform.shape) * 1e-5
        )
        ...
        self._f0_world = f0
        self._sp = pw.cheaptrick(
            x, f0, t, samplerate, fft_size=fft_size
    ...
    def _kth_harmonic(self, k: int) -> np.ndarray:
        ...
        waveform = (
            torch.from_numpy(self.harmonic()).unsqueeze(0).to(device)
        )  # [B, n_samples]
        ...
        if pad_size > 0:
            f0 = np.pad(
                f0, (0, pad_size), mode="constant", constant_values=(f0[0], f0[-1])
            )
        ...
        phase = (
            torch.arange(win_size, dtype=waveform.dtype, device=device)
            / win_size
            * 2
            * np.pi
        )
    ...
    def harmonic(self, k: int = None) -> np.ndarray:
        ...
        self._harmonic_part = pw.synthesize(
            self._f0_world,
            np.clip(
                self._sp * (1 - self._ap * self._ap), a_min=1e-16, a_max=None
            ),  # clip to avoid zeros
